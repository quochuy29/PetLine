...
on-app-latest: ## On app without elastic search, always build latest app image (dev build)
	uid=`id -u` gid=`id -g` docker-compose up --build --detach --remove-orphans

on-app:  ## On app without elastic search
...
build-prod: ## Docker build coverage production image
	...
	\
	docker tag petline-public:$${TAG} $${AWS_ACCOUNT_ID}.dkr.ecr.ap-northeast-1.amazonaws.com/petline-public:$${TAG};\
	docker tag petline-app:$${TAG} $${AWS_ACCOUNT_ID}.dkr.ecr.ap-northeast-1.amazonaws.com/petline-app:$${TAG};\
...
test: on-app## Run phpunit test app integration with database
	docker exec -t petline_app tests/phpunit --no-coverage --testdox -c tests/phpunit.xml

test-library: on-app ## Run phpunit test library only - testdox
	docker exec -t petline_app tests/phpunit --no-coverage --testdox -c tests/phpunit.lib.xml

test-petline: on-app ## Measure test coverage
	docker exec -t -e XDEBUG_MODE=coverage petline_app tests/phpunit -d memory_limit=-1 -c tests/phpunit.xml

clean-test-petline: ## Clean test coverage results
	rm -rf tests/petline

ssh-app: ## SSH into petline app container
	docker exec -it petline_app sh

ssh-app-as-root: ## SSH into petline app container as root user
	docker exec -it --user 0 petline_app sh

show-log-app: ## Show log petline app container
	docker logs -f petline_app

show-log-app-nginx: ## Show log petline nginx container
	docker logs -f petline_public

show-log-db: ## Show log mysql server container
	docker logs -f petline_db

# show-log-saml: ## Show log saml server - keycloak container
# 	docker logs -f keycloak
npm-run: ## Run NPM run in APP container
	docker exec -t --user 0 petline_app npm run watch

check-eslint: ## Run check eslint in source
	docker exec -t --user 0 petline_app npm run eslint


composer-install: ## Run Composer install in APP container
	docker exec -t petline_app composer install

npm-install: ## Run NPM install in APP container
	rm -rf node_modules && docker exec -t --user 0 petline_app npm install
